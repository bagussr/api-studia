...
@tugas_route.post("/{kelas_id}", response_model=Tugas)
def create_tugas_kelas(kelas_id: str, tugas: TugasCreate, db: Session = Depends(get_db)):
    return create_tugas_kelas(db=db, tugas=tugas, kelas_id=kelas_id)


@tugas_route.get("/{kelas_id}", response_model=List[Tugas])
